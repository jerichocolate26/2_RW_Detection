...
# Perform inference using a CNN model and map predictions to class names
def perform_inference(model, spectrogram):
    spectrogram = np.expand_dims(spectrogram, axis=0)  # Add batch dimension
    prediction = model.predict(spectrogram)  # Perform inference (output probabilities)
    ...
    class_name = class_mapping.get(predicted_index, "Unknown")  # Default to "Unknown" if index not found
